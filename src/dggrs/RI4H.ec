...
/*                                                              dk = td (for next step)
I4H                                                             tk for two steps (= dkdk = tdtd); dktk = tkdk                (m + n)^2 − mn       10T + 2
level GP notation Name                             Class       Conway                                                            T                 Count
   0: GP(1,0)     dodecahedron                     1           D     dI                  dI               D                D     1                    12
   1: GP(2,0)     chamfered dodecahedron           1                                                                             4                    42
   2: GP(4,0)                                      1                                                                             16                  162
   3: GP(8,0)                                      1                                                                             64                  642
   4: GP(16,0)                                     1                                                                             256                2562
*/
...
public class RhombicIcosahedral4H : DGGRS
{
   RI5x6Projection pj;
   bool equalArea;
   ...
   uint64 countSubZones(I4HZone zone, int depth)
   {
      return zone.getSubZonesCount(depth);
   }

   int getZoneLevel(I4HZone zone)
   {
      return zone.level;
   }

   int countZoneEdges(I4HZone zone) { return zone.nPoints; }

   bool isZoneCentroidChild(I4HZone zone)
   {
      return zone.isCentroidChild;
   }

   __attribute__ ((optimize("-fno-unsafe-math-optimizations")))
   double getZoneArea(I4HZone zone)
   {
      double area;
      if(equalArea)
      {
   ...
   I4HZone getZoneFromCRSCentroid(int level, CRS crs, const Pointd centroid)
   {
      if(level <= 33)
      {
         switch(crs)
         {
            case 0: case CRS { ogc, 153456 }:
               return I4HZone::fromCentroid(level, centroid);
            case CRS { ogc, 1534 }:
            {
               Pointd c5x6;
               RI5x6Projection::fromIcosahedronNet({ centroid.x, centroid.y }, c5x6);
               return I4HZone::fromCentroid(level, { c5x6.x, c5x6.y });
            }
            case CRS { epsg, 4326 }:
            case CRS { ogc, 84 }:
               return (I4HZone)getZoneFromWGS84Centroid(level,
                  crs == { ogc, 84 } ?
   ...
   int getZoneNeighbors(I4HZone zone, I4HZone * neighbors, I3HNeighbor * nbType)
   {
      return zone.getNeighbors(neighbors, nbType);
   }

   I4HZone getZoneCentroidParent(I4HZone zone)
   {
      return zone.centroidParent;
   }

   I4HZone getZoneCentroidChild(I4HZone zone)
   {
      return zone.centroidChild;
   }

   int getZoneParents(I4HZone zone, I4HZone * parents)
   {
      return zone.getParents(parents);
   }

   int getZoneChildren(I4HZone zone, I4HZone * children)
   {
      return zone.getChildren(children);
   }

   // Text ZIRS
   void getZoneTextID(I4HZone zone, String zoneID)
   {
      zone.getZoneID(zoneID);
   }

   I4HZone getZoneFromTextID(const String zoneID)
   {
      return I4HZone::fromZoneID(zoneID);
   }

   // Sub-zone Order
   I4HZone getFirstSubZone(I4HZone zone, int depth)
   {
   ...
   void compactZones(Array<DGGRSZone> zones)
   {
      ...
      AVLTree<I4HZone> zonesTree { };

      for(i = 0; i < count; i++)
      {
         I4HZone zone = (I4HZone)zones[i];
         if(zone != nullZone)
         {
   ...
   int64 getSubZoneIndex(I4HZone parent, I4HZone subZone)
   {
      int64 ix = -1;
   ...
   DGGRSZone getSubZoneAtIndex(I4HZone parent, int relativeDepth, int64 index)
   {
      I4HZone subZone = nullZone;
      if(index >= 0 && index < countSubZones(parent, relativeDepth))
      {
         ...
         else
         {
            ...
            subZone = I4HZone::fromCentroid(parent.level + relativeDepth, centroid);
         }
      }
      return subZone;
   }

   I4HZone getZoneFromWGS84Centroid(int level, const GeoPoint centroid)
   {
      if(level <= 33)
      {
         Pointd v;
         pj.forward(centroid, v);
         return I4HZone::fromCentroid(level, v);
      }
      return nullZone;
   }

   void getZoneCRSCentroid(I4HZone zone, CRS crs, Pointd centroid)
   {
      switch(crs)
      {
         case 0: case CRS { ogc, 153456 }:
   ...
   void getZoneWGS84Centroid(I4HZone zone, GeoPoint centroid)
   {
      pj.inverse(zone.centroid, centroid, zone.subHex > 2);
   }

   void getZoneCRSExtent(I4HZone zone, CRS crs, CRSExtent extent)
   {
      switch(crs)
      {
         case 0: case CRS { ogc, 153456 }:
   ...
   void getZoneWGS84Extent(I4HZone zone, GeoExtent extent)
   {
      int i;
      GeoPoint centroid;
   ...
   int getZoneCRSVertices(I4HZone zone, CRS crs, Pointd * vertices)
   {
   ...
   int getZoneWGS84Vertices(I4HZone zone, GeoPoint * vertices)
   {
      Pointd v5x6[6];
   ...
   Array<Pointd> getZoneRefinedCRSVertices(I4HZone zone, CRS crs, int edgeRefinement)
   {
      if(crs == CRS { ogc, 1534 })
   ...
   Array<GeoPoint> getZoneRefinedWGS84Vertices(I4HZone zone, int edgeRefinement)
   {
      return (Array<GeoPoint>)getRefinedVertices(zone, { epsg, 4326 }, edgeRefinement, true);
   }

   void getApproxWGS84Extent(I4HZone zone, GeoExtent extent)
   {
      int sh = zone.subHex;
      int i;
      GeoPoint centroid;
   ...
   private static Array<Pointd> getRefinedVertices(I4HZone zone, CRS crs, int edgeRefinement, bool useGeoPoint) // 0 edgeRefinement for 1-20 based on level
   {
   ...
   // Sub-zone Order
   Array<Pointd> getSubZoneCRSCentroids(I4HZone parent, CRS crs, int depth)
   {
   ...
   Array<GeoPoint> getSubZoneWGS84Centroids(I4HZone parent, int depth)
   {
   ...
   static Array<DGGRSZone> listZones(int zoneLevel, const GeoExtent bbox)
   {
      ...
      for(yi = 0; yi < yCount; yi++)
      {
         ...
         for(xi = 0; xi < xCount; xi++)
         {
            ...
            if(rootX < 5 && (d == 0 || d == 1))
            {
               ...
               hexes[nHexes++] = I4HZone::fromI9R(i9RLevel, row, col, hexSubLevel ? 'D' : 'A');
               if(hexes[nHexes-1] == nullZone)
                  continue; // This should no longer happen...

               if(hexSubLevel)
               {
                  hexes[nHexes++] = I4HZone::fromI9R(i9RLevel, row, col, 'E');
                  hexes[nHexes++] = I4HZone::fromI9R(i9RLevel, row, col, 'F');
               }
      ...
      // "North" pole
      tsZones.Add(I4HZone::fromI9R(i9RLevel, 0, (uint)(power-1), hexSubLevel ? 'G' : 'B'));
      // "South" pole
      tsZones.Add(I4HZone::fromI9R(i9RLevel, (uint)(6*power-1), (uint)(4*power), hexSubLevel ? 'H' : 'C'));
...
public class I4HZone : private DGGRSZone
{
public:
   uint levelI4R:5:58;  // 0 .. 16
...
private:
   property int level
   {
      get { return levelI4R * 2 + (subHex > 2); }
   }
   ...
   I4HZone ::fromZoneID(const String zoneID)
   {
      I4HZone result = nullZone;
      char levelChar;
      uint root;
      uint64 ix;
      char subHex;
   ...
   I4HZone ::fromI9R(int level, uint row, uint col, char subHex)
   {
      uint64 p = POW3(level);
   ...
   bool ::validate(uint levelI4R, uint row, uint col, char subHex)
   {
      uint64 p = POW3(levelI4R);
      uint rowOP = (uint)(row / p), colOP = (uint)(col / p);
      int y = (int)(row - rowOP * p), x = (int)(col - colOP * p);
   ...
   property I4HZone parent0
   {
      get
      {
         ...
         else
         {
            ...
            {
               int row, col, level = iLRCFromLRtI((char)('A' + levelI4R), rootRhombus, rhombusIX, &row, &col);
               uint64 p = POW3(level);
   ...
   I4HZone getNeighbor(I3HNeighbor which)
   {
   ...
   int getNeighbors(I4HZone neighbors[6], I3HNeighbor i3hNB[6])
   {
      int numNeighbors = 0;
      I3HNeighbor n;
   ...
   property I4HZone centroidParent
   {
      get
      {
         I4HZone cParent = parent0;
   ...
   int getContainingGrandParents(I4HZone cgParents[3])
   {
      I4HZone cParent = centroidParent;
      int n;
   ...
   int getParents(I4HZone parents[3])
   {
      I4HZone parent0 = this.parent0;
   ...
   I4HZone ::fromCentroid(uint level, const Pointd centroid) // in RI5x6
   {
      Pointd c = centroid;
      uint l9r = level / 2;
      uint64 p = POW3(l9r);
   ...
   {
      int sh = subHex;
      uint level = levelI4R, root = rootRhombus;
   ...
   property I4HZone centroidChild
   {
      get
      {
         if(this == nullZone)
         ...
         {
            I4HZone centroidChild;
            uint l9r = levelI4R, sh = subHex;
            ...
            else
            {
               ...
               switch(sh)
               {
                  ...
                  default:
                  {
                     ...
                     centroidChild = I4HZone::fromI9R(l9r + 1, r, c, 'A');
                     break;
                  }
               }
            }
   ...
   int getChildren(I4HZone children[7])
   {
      uint l9r = levelI4R, sh = subHex;
      int i = 0;
   ...
   I4HZone getFirstSubZone(int rDepth)
   {
   ...
   private /*static */bool orderZones(int zoneLevel, AVLTree<I4HZone> tsZones, Array<I4HZone> zones)
   {
      ...
      if(centroids)
      {
         ...
         {
            I4HZone key = I4HZone::fromCentroid(zoneLevel, centroids[i]);
            if(tsZones.Find(key))
               zones.Add(key);
            else
            {
...
static void compactI3HZones(AVLTree<I4HZone> zones, int level)
{
   AVLTree<I4HZone> output { };
   AVLTree<I4HZone> next { };
   int l;
         ...
         for(p = 0; p < nCGParents; p++)
         {
            ...
            if(gParent != nullZone && !next.Find(gParent))
            {
               ...
               if(parentAllIn)
               {
                  ...
                  {
                     I4HZone ch = children[i];
                     if(ch != nullZone)
                     {
                        I4HZone cChild = ch.centroidChild;

                        if(!zones.Find(cChild))
                        {
                           Pointd cv = cChild.centroid;
                           int cl = 2*cChild.levelI4R + (cChild.subHex >= 3);
                           I4HZone sub = I4HZone::fromCentroid(cl + 2, cv);
                           if(!output.Find(sub))
                              parentAllIn = false;
                        }
                     }
...
static bool findSubZone(const Pointd szCentroid, int64 index, const Pointd c)
{
   ...
   if(fabs(centroid.x - szCentroid.x) < 1E-11 &&
      ...
      return false;
   return true;
   // return *zone != I4HZone::fromCentroid(zone->level, centroid);
}

static void getIcoNetExtentFromVertices(I4HZone zone, CRSExtent value)
{
   int i;
...
static Array<Pointd> getIcoNetRefinedVertices(I4HZone zone, int edgeRefinement)   // 0 for 1-20 based on level
{
   Array<Pointd> rVertices = null;
   Pointd vertices[9];
